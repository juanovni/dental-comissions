<x-filament-panels::page>
    ...
    <div class="dq-page">
        ...
        <div class="dq-grid">
            <main style="display: grid; gap: 1rem;">
                <section class="dq-card">
                    ...
                    @if ($next)
                        ...
                        @if ($next->notes)<div class="dq-note">NOTA · {{ $next->notes }}</div>@endif
                        <div class="dq-actions">
                            ...
                            @endif
                            <button class="dq-action" type="button">Ver contacto</button><button class="dq-action" type="button">Nota</button>
                        </div>
                    @else
            ...
            <aside class="dq-side">
                <section class="dq-panel"><div class="dq-title">En consulta</div>@if ($current)<div class="dq-patient"><strong>{{ $current->patient?->full_name ?? 'Paciente sin nombre' }}</strong><span class="dq-meta">{{ $current->procedure?->name ?? 'Sin procedimiento' }} · {{ $current->consultationMinutes() ?? 0 }} min</span><button class="dq-action dq-action-primary" wire:click="transition({{ $current->id }}, '{{ \App\Enums\AppointmentStatus::Completed->value }}')">Finalizar consulta</button></div>@else <div class="dq-meta">Sin paciente en consulta.</div>@endif</section>
